...
if (empty($_SESSION['products'])) {
    ...
    if (count($_SESSION['products']) === 1) {
        ...
        unset($_SESSION['products'][key($_SESSION['products'])]);
    } else {
        // 配列の中身がなくなるまでunsetで回す
        while (!empty($_SESSION['products'])) {
            unset($_SESSION['products'][key($_SESSION['products'])]);
        }
    }
}
...
<?php foreach($products as $name => $product): ?>
    <tr>
        ...
        <td label="個数：" class="text-right"><div class ="left"> <?php echo $product['count']; ?></div></td>
...
<div class="cart-btn">
    <button type="button"  class="btn-sm btn-blue" onclick="location.href='kounyu.php'">購入</button>
    <button type="buttpn"  class="btn-sm btn-blue" onclick="location.href='shopping2.php'">お買い物を続ける</button>
   </div>
  </div>
 </div>
</div>
